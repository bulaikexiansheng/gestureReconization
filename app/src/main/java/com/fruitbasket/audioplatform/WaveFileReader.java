    ...
    private boolean issuccess = false;
        ...
        private void initReader(String filename) {
            ...
            try {
                ...
                this.chunkdescriptor = readString(Constents.LENCHUNKDESCRIPTOR);
                if (!chunkdescriptor.endsWith("RIFF"))
                    throw new IllegalArgumentException("RIFF miss, " + filename + " is not a wave file.");

                this.chunksize = readLong();
                this.waveflag = readString(Constents.LENWAVEFLAG);
                if (!waveflag.endsWith("WAVE"))
                    throw new IllegalArgumentException("WAVE miss, " + filename + " is not a wave file.");

                this.fmtubchunk = readString(Constents.LENFMTSUBCHUNK);
                ...
                this.datasubchunk = readString(Constents.LENDATASUBCHUNK);
                ...
                for (int i = 0; i < this.len; ++i) {
                    for (int n = 0; n < this.numchannels; ++n) {
                        if (this.bitspersample == 8) {
                            ...
                            this.data[n][i] = (double)(this.readInt());
                        }
                    }
                }
